## Sicherheitsregeln
- NIEMALS .env-Dateien oder andere Dateien mit Secrets lesen
- NIEMALS Passwörter, API-Keys oder andere sensitive Daten anzeigen
- Bei Debugging nur anonymisierte oder maskierte Ausgaben verwenden
- Bei Datenbank-Problemen alternative Debugging-Methoden verwenden

## Bausteine-spezifische Regeln
- **Repository-Struktur** - Jeder Baustein hat ein eigenes GitHub-Repository
- **Deployment-Strategie** - Telekom für Frontend, Vercel für Backend-Bausteine
- **Domain-Struktur** - auth.revalenz.de, calendar.revalenz.de, support.revalenz.de
- **Integration-Pattern** - Bausteine kommunizieren über standardisierte APIs
- **Dokumentation-Standard** - Jeder Baustein braucht README mit 5-Minuten-Setup
...
## Bausteine-Architektur Spezifika

### Modular Building Block Philosophy

When working with the Revalenz Bausteine-System:

- **Einsteiger-freundlich** - Jeder Baustein muss für Webentwickler ohne tiefe Kenntnisse verständlich sein
- **Plug-and-Play** - Bausteine müssen ohne komplexe Konfiguration funktionieren
- **Dokumentation-first** - Jeder Baustein braucht klare, schrittweise Anleitungen
- **Minimale Abhängigkeiten** - Bausteine sollen unabhängig voneinander funktionieren

### Baustein-Design Prinzipien

1. **Single Responsibility** - Ein Baustein = eine Funktion (Auth, Calendar, Tickets)
2. **Standard Interfaces** - Konsistente API-Patterns across alle Bausteine
3. **Progressive Enhancement** - Bausteine funktionieren auch ohne andere Bausteine
4. **Hosting Agnostic** - Bausteine sollen auf verschiedenen Plattformen deploybar sein

### Einsteiger-Onboarding

Für neue Entwickler:
- **5-Minuten Setup** - Jeder Baustein muss in 5 Minuten lauffähig sein
- **Copy-Paste Examples** - Funktionsfähige Code-Beispiele für sofortigen Start
- **Visual Documentation** - Screenshots, GIFs, und Diagramme für komplexe Konzepte
- **Troubleshooting Guides** - Häufige Probleme und Lösungen dokumentiert

### Community-Integration

- **Open Source Ready** - Bausteine sollen von anderen Organisationen übernommen werden können
- **Multilingual Support** - Deutsche und englische Dokumentation
- **Sovereignty Principles** - Keine Vendor-Lock-ins, vollständige Kontrolle über Code und Daten
